...
import net.encdec.edcmd.res_files.GetResFile;
...
public class LogFiles {
	...
	private static File edconfig = new File(cmd.getAbsolutePath(), "edconfig");
	...
	public void logFiles() {
		try {
			...
			if (!logConf.exists()) {
				new GetResFile().loadfile(getLogs().getAbsolutePath(), 1);
				if (logConf.exists()) {
					logFileSet = true;
				}
			} else {
